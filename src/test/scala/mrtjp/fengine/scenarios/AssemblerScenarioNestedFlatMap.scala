...
/**
  ...
  */
class AssemblerScenarioNestedFlatMap extends AssemblerScenario
{
    ...
    private val assembler = ICAssembler.newAssembler
    assembler.addTileMap(submap, Map.empty)
    private val subflatmap = assembler.result()
    ...
    private val map = new TTestFETileMap {
        ...
        val nestedTile = new PortlessNestedFlatMapTile("N", bitNorth, bitSouth, Map((dirNorth, submap.inputTileRegID)), Map((dirSouth, submap.outputTileRegID)), subflatmap)
